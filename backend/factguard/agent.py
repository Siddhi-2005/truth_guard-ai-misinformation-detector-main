...
SYSTEM_PROMPT = """
You are FactGuard — an autonomous, evidence‑first verification agent built for FactGuard.
...
factguard_researcher = Agent(
    name="factguard_researcher",
    model=config.model,
    description="Evidence-first verification researcher.",
    instruction=SYSTEM_PROMPT,
    tools=[google_search],
)

factguard_formatter = Agent(
    name="factguard_formatter",
    model=config.model,
    description="Evidence-first verification formatter.",
    instruction="""You are the formatting assistant for FactGuard.
Your job is to read the preceding research report (which was generated by the researcher agent) and structure it strictly into the required JSON schema.
Output a valid JSON object matching the FactGuardOutput schema. Do not run any search tools.""",
    output_schema=FactGuardOutput,
)

factguard_agent = SequentialAgent(
    name="factguard_agent",
    description="Evidence-first verification agent.",
    sub_agents=[factguard_researcher, factguard_formatter],
)

CHAT_SYSTEM_PROMPT = """
You are FactGuard — a helpful and evidence-first AI assistant for FactGuard.
...
factguard_chat_responder = Agent(
    name="factguard_chat_responder",
    model=config.model,
    description="Conversational responder for FactGuard.",
    instruction=CHAT_SYSTEM_PROMPT,
    tools=[google_search],
)

factguard_chat_formatter = Agent(
    name="factguard_chat_formatter",
    model=config.model,
    description="Conversational formatter for FactGuard.",
    instruction="""You are the formatting assistant for FactGuard Chat.
Your job is to read the preceding conversational response and structure it strictly into the required JSON schema.
Assess the nature of the conversation query:
...
Output a valid JSON object matching the ChatOutput schema. Do not run any search tools.""",
    output_schema=ChatOutput,
)

factguard_chat_agent = SequentialAgent(
    name="factguard_chat_agent",
    description="Conversational interface for FactGuard.",
    sub_agents=[factguard_chat_responder, factguard_chat_formatter],
)
